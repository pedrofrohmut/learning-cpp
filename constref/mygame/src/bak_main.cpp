...
std::string get_data_path_for(const std::string &file_path) // Pass by ref but const so the value wont change in the function
{
...
struct MyResources
{
    const int ANIM_PLAYER_IDLE = 0; // Represents the index animation in the player animations vector
    ...
    void load(MyGame &game)
    {
        this->playerAnimations.resize(5);
        this->playerAnimations[ANIM_PLAYER_IDLE] = Animation(8, 1.6f);

        this->texture_idle = this->load_texture(game.renderer, get_data_path_for("idle.png"));
    }
